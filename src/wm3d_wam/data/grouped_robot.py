"""Unified, embodiment-aware robot state and action contract for WM3D-WAM.
...
GROUPED_ROBOT_SCHEMA = "wm3d_wam_grouped_robot_v1"
...
def pack_grouped_robot_window(
    *,
    ...
    fine_dt = np.zeros(
        (interval_count, limits.max_groups, limits.max_substeps), dtype=np.float64
    )
    ...
    for series in action_series:
        ...
        if series.supervision == "fine_command":
            ...
            for row, timestamp_s in enumerate(timestamps.tolist()):
                ...
                fine_dt[interval, group_slot, substep] = (
