...
def test_data_config_keeps_renderer_and_action_clocks_separate() -> None:
    ...
    assert (
        config.action.padded_capacity_rule == "ceil_duration_times_source_hz_plus_one"
    )
...
def test_runtime_forbids_gpu_zero_and_geometry_bridge_is_a_hard_gate() -> None:
    ...
    assert list(runtime.runtime.permitted_cuda_devices) == [1, 2, 3, 4]
    assert list(runtime.runtime.visible_cuda_devices) == [1, 2, 3, 4]
    assert list(runtime.runtime.forbidden_cuda_devices) == [0, 5, 6, 7]
    ...
    assert runtime.runtime.effective_global_batch == 16
    ...
    assert runtime.stages.wan_action.gradient_accumulation_steps == 1
    assert runtime.stages.wan_action.effective_global_batch == 4
    assert runtime.stages.wan_action.rejected_gradient_accumulation_steps == 4
    ...
    assert (
        geometry.history_connector._target_ == "wm3d_wam.models.GroupedHistoryConnector"
    )
