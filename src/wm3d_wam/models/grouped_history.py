...
class GroupedStateHistoryCodec(nn.Module):
    ...
    def forward(self, batch: GroupedStateHistoryBatch) -> torch.Tensor:
        ...
        scalar = value_features + metadata[:, None]
        ...
        field_basis = torch.tanh(self.field_binding_down(metadata)).unsqueeze(1)
        compact_binding = field_basis * torch.tanh(values.unsqueeze(-1))
        compact_binding = (
            (compact_binding * weights).sum(dim=(2, 3)) / denominator
        )
        token = token + self.field_binding_up(compact_binding)
