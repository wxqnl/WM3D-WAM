...
class WanVideoVAE(nn.Module):
    ...
    def __init__(self, z_dim=16):
        ...
        # These fixed normalization constants are not checkpoint parameters.
        # Keep real CPU storage even when the module is constructed under a
        # meta-device context for low-memory checkpoint materialization.
...
class WanVideoVAE38(WanVideoVAE):
    ...
    def __init__(self, z_dim=48, dim=160):
        ...
        # See WanVideoVAE above: plain attributes would otherwise inherit the
        # surrounding meta device and remain impossible to materialize.
        self.mean = torch.tensor(mean, device="cpu")
        self.std = torch.tensor(std, device="cpu")
        self.scale = [self.mean, 1.0 / self.std]
